function SCC = DICE2013R_mc(perturbs)
...
Params = set_parameter_perturbation(Params, perturbs); %Perturb parameters
...
for i = 1:N
  ...
  [fi, hi] = dice_dynamics(x_guess(:,i),u_guess(:,i),i,Params); %fi = [T;M;K;J], hi = [E; C]
  x_guess(:, i+1) = fi;
  v_guess(:, i)   = hi;
end
v_guess(:, N+1)   = hi;
...
% Gradient, Hessian for sens stuff
% grad_eq = jacobian(eq_con, [x;u;v]);
% hess_obj = hessian(obj, [x;u;v]);
% 
% grad_func = Function('gr', {[x;u;v]}, {grad_eq});
% hess_func = Function('he', {[x;u;v]}, {hess_obj}); 
% 
% grad_num = full(grad_func(full(res.x))); 
% hess_num = full(hess_func(full(res.x)));

% grad_obj = jacobian(obj, [x;u;v]);
% objfunc = Function('he', {[x;u;v]}, {grad_obj});
% obj_num = full(objfunc(full(res.x)))

% ob = gradient(eq_con(7,end), [x;u;v]);
% obfunc = Function('ob', {[x;u;v]}, {ob}); 
% obnum = full(obfunc(full(res.x)));
